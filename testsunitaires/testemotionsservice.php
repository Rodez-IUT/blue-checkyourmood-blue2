...
class EmotionsServiceTest extends TestCase {
    ...
    public function testGetEmotions() {
        // Given : une instance de la classe EmotionsService et une connexion PDO
        $pdo = $this->getPDO();
        // When : on vérifie si le tableau retourné n'est pas égal à la chaîne de caractères vide
        $aTester = emotionsservice::getEmotions($pdo);
        

        // $this->assertNotEquals('Ne doit pas être égal', json_encode($aTester));    ANCIEN TEST TRIVIAL !!!

        // Then : on vérifie si le tableau retourné correspond bien à un tableau JSON de toutes les émotions avec leur ID, leur emoji et leur nom.
